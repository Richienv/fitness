...
export default function WorkoutHome() {
  ...
  // Remembered machines (staples) + their rank, recomputed when the store loads.
  const machinePicks = useMemo<MachinePick[]>(() => getMachinePicks(), [workouts]);
  const pickRank = useMemo(() => getPickRank(), [workouts]);
  const matchedMachines = useMemo(
    () =>
      query.trim() ? searchEquipment(query, EQUIPMENT, pickRank).slice(0, 8) : [],
    [query, pickRank]
  );
  // Recovery-aware "what to do today" (empty-query only).
  ...
  // One tappable machine row (＋ CATAT) — shared by search, HARI INI, SERING.
  const machineRow = (e: Equipment) => (
    <button
      key={e.id}
      type="button"
  ...
  return (
    ...
    >
      ...
      <div className="workout-home-bottom">
        ...
        {query.trim() && matchedMachines.length > 0 && (
          <>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 22 }}>
              {matchedMachines.map(machineRow)}
            </div>
          </>
        )}
        ...
        {/* ── SERING DIPAKAI — remembered machines (empty query only) ── */}
        {!query.trim() && machinePicks.length > 0 && (
          <>
            <div className="wo-pick-label">⭐ SERING DIPAKAI</div>
            <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 22 }}>
              {machinePicks
                .slice(0, 6)
                .map((p) => getEquipment(p.id))
